...
![Full pipeline benchmark](docs/benchmark_full_pipeline.png)

*Five-backend comparison on GTX 1650 — left: latency (avg and p95 tail);
right: throughput with speedup vs PyTorch. INT8 wins outright; FP16 unexpectedly
appears slower than FP32. The next section explains why.*
...
![GPU vs CPU time breakdown](docs/benchmark_gpu_breakdown.png)

*The actual GPU inference (colored segment, measured via CUDA Events) is 3–6 ms across
all TensorRT precisions. The 94–97% above is CPU preprocessing, CPU↔GPU transfers, and
CPU postprocessing. The 13 ms "FP16 penalty" in the previous chart was CPU jitter
swamping the genuine 2 ms GPU advantage — at the GPU level the ordering is exactly
INT8 → FP16 → FP32, as theory predicts.*
